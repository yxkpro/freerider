				...
				for (const line of this.constructor.physics) {
					for (let i = 0; i < line.length; i += 2) {
						if (event.shiftKey && event.ctrlKey){
							line[i] = line[i] * (1 / 1.5) / 25 * 25; 
							line[i + 1] = line[i + 1] * (1 / 1.5) / 25 * 25;
						}
						else if (event.ctrlKey){
							line[i] = line[i] * 1.5 / 25 * 25; 
							line[i + 1] = line[i + 1] * 1.5 / 25 * 25;
						}
			...
			case 'r': {
				...
				for (const line of this.constructor.physics) {
					for (let i = 0, e; i < line.length; i += 2) {
						if (event.shiftKey && event.ctrlKey){
						e = line[i];
						line[i] = (Math.cos(15 * Math.PI / 180) * e + Math.sin(15 * Math.PI / 180) * line[i + 1] / 25 * 25);
						line[i + 1] = (-Math.sin(15 * Math.PI / 180) * e + Math.cos(15 * Math.PI / 180) * line[i + 1] / 25 * 25);
						}
						else if (event.ctrlKey){
						e = line[i];
						line[i] = (Math.cos(15 * -Math.PI / 180) * e + Math.sin(15 * -Math.PI / 180) * line[i + 1] / 25 * 25);

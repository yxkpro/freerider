...
export default class {
	...
	draw() {
		...
		const uniqueSceneryLines = this.removeDuplicateLines(this.scenery.concat(...this.objects.map(({ scenery }) => scenery)));
		for (const line of uniqueSceneryLines) {
		...
		const uniquePhysicsLines = this.removeDuplicateLines(this.physics.concat(...this.objects.map(({ physics }) => physics)));
		for (const line of uniquePhysicsLines) {
			if (line.length < 4) continue;
			this.ctx.beginPath();
			this.ctx.moveTo(line[0], line[1]);
			for (let i = 2; i < line.length; i += 2) {
	...
	removeDuplicateLines(lines) {
		...
		for (const line of lines) {
			const lineString = line.map(coord => coord.toString(32)).join(' ');
			const reversedLineString = line.slice(2).concat(line.slice(0, 2)).map(coord => coord.toString(32)).join(' ');
			if (!seen.has(lineString) && !seen.has(reversedLineString)) {
				seen.add(lineString);
				seen.add(reversedLineString);
	...
	toString() {
		...
		for (const line of Array(...miniPhysics, ...miniScenery)) {
		  for (let i = 0; i < line.length; i += 2) {
			line[i] = Math.floor(line[i] * 0.6) + 26000;
			line[i + 1] = Math.floor(line[i + 1] * 0.6);
		  }
		}
		...
		const uniquePhysics = removeDuplicateLines(physics);
		const uniqueScenery = removeDuplicateLines(scenery);
	  
		if (event.ctrlKey) {
		  return Array(uniquePhysics.map(line => line.map(coord => coord.toString(32)).join(' ')).join(','), uniqueScenery.map(line => line.map(coord => coord.toString(32)).join(' ')).join(','), this.powerups || '').join('#');
		} else {
		  return Array(uniquePhysics.map(line => line.map(coord => coord.toString(32)).join(' ')).join(','), uniqueScenery.map(line => line.map(coord => coord.toString(32)).join(' ')).join(','), this.powerups || '').join('#');
		}
	  }

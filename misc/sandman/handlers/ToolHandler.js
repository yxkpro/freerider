...
export let gridwidth; // Declare width variable for export
...
for (const id in ObjectDictionary) {
  ...
  gridwidth = Math.abs(Math.max(...flatX) - Math.min(...flatX));
  const flatY = joined.flatMap(lines => lines.filter((_, index) => index % 2 === 1));
  const height = Math.min(...flatY) + Math.max(...flatY);
  gridheight = Math.abs(Math.max(...flatY) - Math.min(...flatY));
  ...
  ObjectDictionary[id] = class extends ObjectTool {
    ...
    isCustom = false; // Add a property to indicate if it's a custom object or not
  };
}
